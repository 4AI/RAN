...
class Trainer(object):
    ...
    def train_model(self, local_model, x_train, y_train, f_train=None,
        ...
        if self.checkpoint_path is not None:
            # save the config, preprocessor on the disk
            print('save the config, preprocessor on the disk')
            self.model_config.save(os.path.join(self.checkpoint_path, CONFIG_FILE_NAME))
            self.preprocessor.save(os.path.join(self.checkpoint_path, PROCESSOR_FILE_NAME))
