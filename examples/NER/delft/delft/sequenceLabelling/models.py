from tensorflow.keras.layers import Dense, LSTM, GRU, Bidirectional, Embedding, Input, Dropout, Reshape, Lambda
...
def get_model(config: ModelConfig, preprocessor, ntags=None, load_pretrained_weights=True, local_path=None):
    ...
    if config.architecture == BidLSTM.name:
    ...
    elif config.architecture == RanNet_CNN_CRF.name:
        preprocessor.return_word_embeddings = True
        preprocessor.return_chars = True
        preprocessor.return_lengths = True
        preprocessor.with_mask = True
        config.use_crf = True
        return RanNet_CNN_CRF(config, ntags)

    elif config.architecture == RanNet_CNN_CRF_CASING.name:
        preprocessor.return_word_embeddings = True
        preprocessor.return_chars = True
        preprocessor.return_casing = True
        preprocessor.return_lengths = True
        preprocessor.with_mask = True
        config.use_crf = True
        return RanNet_CNN_CRF_CASING(config, ntags)
...
class RanNet_CRF(BaseModel):
    """
    A Keras implementation of RanNet for sequence labelling.
    ...
    def __init__(self, config, ntags=None):
        ...
        chars = TimeDistributed(Bidirectional(LSTM(config.num_char_lstm_units, return_sequences=False)))(char_embeddings)
        mask_input = Input(shape=(None, ), dtype='int32', name='mask_input')
        mask = mask_input
        ...
        output = x
        cell = None
        for i in range(config.ran_layers):
            inputs = [output, mask, cell] if cell is not None else [output, mask]
            output, cell = RAN(
                config.ran_head_num,
                config.ran_head_size,
                config.ran_window_size,
                dropout_rate=config.ran_dropout_rate,
                cell_initializer_type=config.ran_cell_initializer,
                kernel_initializer=config.ran_initializer,
                activation='swish',
                name=f'RAN-Layer-{i}'
            )(inputs)
        output = Dense(config.ran_hidden_size, activation='swish')(output)

        base_model = Model(inputs=[word_input, char_input, mask_input, length_input], outputs=[output])
...
class RanNet_CNN_CRF(BaseModel):
    """
    A Keras implementation of RanNet for sequence labelling.
    ...
    def __init__(self, config, ntags=None):
        ...
        char_dropout = Dropout(config.dropout)(char_embeddings)

        conv1d_out = TimeDistributed(
            Conv1D(kernel_size=3, filters=64, padding='same', activation='swish', strides=1))(char_dropout)  # filters 30, swish
        maxpool_out = TimeDistributed(GlobalMaxPooling1D())(conv1d_out)
        chars = Dropout(config.dropout)(maxpool_out)
        ...
        for i in range(config.ran_layers):
            ...
            # w/ residual
            if config.ran_residual and config.ran_layers > 2:
                output = Lambda(lambda x: x[0] + x[1])([output, output_t])
                # output = LayerNorm()(output)
            else:
                output = output_t

        output = Dense(config.ran_hidden_size, activation='swish')(output)  # swish
        # output = Dropout(config.dropout)(output)
...
class RanNet_CNN_CRF_CASING(BaseModel):
    """
    A Keras implementation of RanNet for sequence labelling.
    ...
    def __init__(self, config, ntags=None):
        ...
        char_dropout = Dropout(config.dropout)(char_embeddings)
        conv1d_out = TimeDistributed(
            Conv1D(kernel_size=3, filters=30, padding='same',activation='swish', strides=1))(char_dropout)  # tanh
        maxpool_out = TimeDistributed(GlobalMaxPooling1D())(conv1d_out)
        chars = Dropout(config.dropout)(maxpool_out)
        ...
        base_model = Model(inputs=[word_input, char_input, mask_input, casing_input, length_input], outputs=[output])
