...
class DataGenerator(BaseGenerator):
    ...
    def __getitem__(self, index):
        ...
        '''
        batch_x, batch_c, batch_m, batch_f, batch_a, batch_l, batch_y = self.__data_generation(index)
        ret = [batch_x, batch_c]
        if self.preprocessor.with_mask:
            ret.append(batch_m)
        if self.preprocessor.return_casing:
            return ret + [batch_a, batch_l], batch_y
        elif self.preprocessor.return_features:
            return ret + [batch_f, batch_l], batch_y
        else:
            return ret + [batch_l], batch_y
    ...
    def __data_generation(self, index):
        ...
        if self.embeddings and self.embeddings.use_ELMo:
            batch_x, elmo_token_masks = to_vector_simple_with_elmo(x_tokenized, self.embeddings, max_length_x, extend=extend, with_mask=True)
            batch_m = np.zeros(batch_x.shape[:-1], dtype=np.int32)
            for i, masks in enumerate(elmo_token_masks):
                batch_m[i, :len(masks)] = 1
            # print('>>> batch_m.shape:', batch_m.shape, batch_x.shape, batch_m)
        else:
            batch_x = np.zeros((max_iter, max_length_x, self.embeddings.embed_size), dtype='float32')
            batch_m = np.zeros((max_iter, max_length_x), dtype=np.int32)
            for i in range(0, max_iter):
                batch_x[i], batch_m[i] = to_vector_single(x_tokenized[i], self.embeddings, max_length_x, return_mask=True)
        ...
        return batch_x, batch_c, batch_m, batch_f, batch_a, batch_l, batch_y
