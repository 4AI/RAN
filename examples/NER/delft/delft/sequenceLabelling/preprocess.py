...
def to_vector_single(tokens, embeddings, maxlen, lowercase=False, num_norm=True, return_mask=False):
    """
...
def to_vector_elmo(tokens, embeddings, maxlen, lowercase=False, num_norm=False, extend=False):
    ...
    """
    subtokens, _ = get_subtokens(tokens, maxlen, extend, lowercase)
    return embeddings.get_sentence_vector_only_ELMo(subtokens)


def to_vector_simple_with_elmo(tokens, embeddings, maxlen, lowercase=False, num_norm=False, extend=False, with_mask=False):
    """
    Given a list of tokens convert it to a sequence of word embedding 
    ...
    """
    subtokens, masks = get_subtokens(tokens, maxlen, extend, lowercase)
    embeddings = embeddings.get_sentence_vector_with_ELMo(subtokens)
    if with_mask:
...
def get_subtokens(tokens, maxlen, extend=False, lowercase=False):
    ...
    for i in range(0, len(tokens)):
        ...
        masks.append([1] * len(local_tokens))
        subtokens.append(local_tokens)
    return subtokens, masks
