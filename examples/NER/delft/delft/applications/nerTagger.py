...
def configure(architecture, dataset_type, lang, embeddings_name, use_ELMo):
    batch_size = 32 # 64 # 128 # 512  # 150
    max_sequence_length = 300
    patience = 100 # 5
    early_stop = False
    max_epoch = 100 # 100
    learning_rate = 5e-4 # 3e-4 # 3e-4 # 5e-4  # 1e-3
    multiprocessing = False # True
    # log_dir
    log_dir = f"data/{architecture}-{dataset_type}-{str(datetime.date.today())}-ckpt"
    ...
    return batch_size, max_sequence_length, patience, recurrent_dropout, early_stop, max_epoch, embeddings_name, word_lstm_units, multiprocessing, learning_rate, log_dir
...
def train(dataset_type='conll2003', lang='en', embeddings_name=None, architecture='BidLSTM_CRF', transformer=None, data_path=None, use_ELMo=False):
    ...
    batch_size, max_sequence_length, patience, recurrent_dropout, early_stop, max_epoch, embeddings_name, word_lstm_units, multiprocessing, learning_rate, log_dir = \
    ...
    if (dataset_type == 'conll2003') and (lang == 'en'):
        ...
        x_train2, y_train2 = load_data_and_labels_conll('data/sequenceLabelling/CoNLL-2003/eng.test')  #  recover eng.dev
        x_train3, y_train3 = load_data_and_labels_conll('data/sequenceLabelling/CoNLL-2003/eng.test')
...
def train_eval(embeddings_name=None, 
    ...
    if (dataset_type == 'conll2003') and (lang == 'en'):
        print('Loading CoNLL 2003 data (train_eval)...')
        x_train, y_train = load_data_and_labels_conll('data/sequenceLabelling/CoNLL-2003/eng.train')
        x_valid, y_valid = load_data_and_labels_conll('data/sequenceLabelling/CoNLL-2003/eng.test')  # eval on test set, recover eng.dev 
        x_eval, y_eval = load_data_and_labels_conll('data/sequenceLabelling/CoNLL-2003/eng.test')
        ...
        print('Loading Ontonotes 5.0 CoNLL-2012 NER data (train_eval)...')

        x_train, y_train = load_data_and_labels_conll('data/sequenceLabelling/CoNLL-2012-NER/eng.train')
        x_valid, y_valid = load_data_and_labels_conll('data/sequenceLabelling/CoNLL-2012-NER/eng.test')  # eval on test set, recover dev 
...
def eval(embeddings_name=None, 
    ...
    (batch_size, max_sequence_length, patience, recurrent_dropout,
     early_stop, max_epoch, embeddings_name, word_lstm_units, multiprocessing,
     learning_rate, log_dir) = configure(architecture, dataset_type, lang, embeddings_name, use_ELMo)

    if (dataset_type == 'conll2003') and (lang == 'en'):
        print('Loading CoNLL-2003 NER data...')
        x_test, y_test = load_data_and_labels_conll('data/sequenceLabelling/CoNLL-2003/eng.test')
...
if __name__ == "__main__":
    ...
    architectures_word_embeddings = [
        ...
        'RanNet_CRF', 'RanNet_CNN_CRF', 'RanNet_CNN_CRF_CASING'
     ]
